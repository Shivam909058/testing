...
def generate_blog_with_claude(transcription, frames, timestamps, captions):
    """Generate a high-quality blog using Claude API based on video transcription"""
    try:
        ...
        prompt = f"""
        You are tasked with creating a high-quality blog post based on a video transcript. 
        The video contains an explanation about {transcription['segments'][0]['text'][:100]}...

        Here's the full transcript:
        {transcription['full_text']}

        The video includes several key visual elements. For each image, use the following HTML format EXACTLY:
        <img src="image_N.jpg" alt="image description" style="max-width: 100%; height: auto; margin: 20px 0;" />
        where N is the image number (starting from 0).

        Available images and their contexts:
        {json.dumps(frame_contexts, indent=2)}

        IMPORTANT IMAGE FORMATTING RULES:
        - Always use <img src="image_N.jpg"> format, where N starts from 0
        - Never use <IMAGE:N> format
        - Include descriptive alt text for each image
        - Example: <img src="image_0.jpg" alt="{captions[0]}" style="max-width: 100%; height: auto; margin: 20px 0;" />

        Create a well-structured, professional blog post that:
        1. Has a compelling, SEO-friendly title
        2. Uses the actual content from the transcript
        3. Incorporates the images at appropriate points using the specified format
        4. Maintains the original flow and structure
        5. Includes proper headings and sections

        Format your response as a JSON object with:
        {{
            "title": "A clear, topic-focused title",
            "meta_description": "A 150-160 character description of the content",
            "content": "Your HTML blog content with properly formatted image tags"
        }}
        """
        
        with st.status("Generating blog from transcript..."):
            try:
                response = client.messages.create(
                    model=CLAUDE_MODEL,
                    max_tokens=4000,
                    temperature=0.3,
                    system="You are an expert technical writer. Always use standard HTML image tags (<img src='image_N.jpg'>) for images. Never use custom image tags.",
                    messages=[
                ...
                try:
                    ...
                    for i in range(len(frames)):
                        old_tag = f'<IMAGE:{i+1}>'
                        new_tag = f'<img src="image_{i}.jpg" alt="{captions[i]}" style="max-width: 100%; height: auto; margin: 20px 0;" />'
                    ...
                    st.warning("Could not parse JSON from Claude response. Using fallback extraction.")
        ...
        st.warning("Falling back to offline blog generation.")
...
def extract_audio_and_transcribe(video_path):
    """Extract audio from video and transcribe it using direct audio loading"""
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            with st.status("Extracting audio from video..."):
                ...
                try:
                    ...
                    try:
                        st.warning("Trying fallback method with ffmpeg...")
                        ffmpeg_cmd = [
                        ...
                        if os.path.exists(audio_path):
                            st.success("Fallback audio extraction successful")
                        else:
                            raise Exception("Fallback extraction failed to create audio file")
                    except Exception as ffmpeg_error:
                        st.error(f"Fallback extraction failed: {str(ffmpeg_error)}")
                        raise Exception(f"All audio extraction methods failed")
            
            with st.status("Transcribing audio..."):
                try:
                    st.info("Loading audio file with librosa...")
                    audio_data, sample_rate = librosa.load(audio_path, sr=16000)
                    
                    audio_data = audio_data.astype(np.float32)
                    
                    if np.isnan(audio_data).any() or len(audio_data) < 1000:
                        raise Exception("Invalid audio data detected")
                    
                    st.info("Transcribing audio data...")
                    ...
                    if not result['text'] or len(result['text']) < 10:
                        raise Exception("Transcription produced insufficient text")
                    
                    st.success(f"Transcription complete: {len(result['text'])} characters")
                    ...
                    st.error(f"Transcription error with Whisper: {str(e)}")
                    
                    try:
                        st.warning("Trying alternative transcription method...")
                        
                        from faster_whisper import WhisperModel
                        
                        model_size = "base"
                        st.info(f"Loading faster-whisper model ({model_size})...")
                        ...
                        st.success(f"Alternative transcription complete: {len(full_text)} characters")
                        ...
                        st.error(f"Fallback transcription also failed: {str(fallback_error)}")
                        
                        try:
                            st.warning("Trying last resort transcription...")
                            whisper_cmd = [
                                "whisper", audio_path,
                                "--model", "base",
                                "--output_format", "json",
                            ...
                            json_path = os.path.join(temp_dir, f"{os.path.basename(audio_path)}.json")
                            ...
                            if os.path.exists(json_path):
                                ...
                                raise Exception("Command-line transcription failed to produce output")
                                
                        except Exception as last_error:
                            st.error(f"All transcription methods failed: {str(last_error)}")
                            
                            st.warning("Using minimal transcription to continue the process")
                            return {
...
def main():
    st.set_page_config(page_title="Video to Blog Converter", layout="wide")
